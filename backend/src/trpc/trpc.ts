...
// Protected procedure with JWT auth - with explicit type annotation
const _protectedProcedure = t.procedure.use(async ({ ctx, next }) => {
  if (!ctx.userId) {
...
// Export with explicit type that preserves the enhanced context
